...
<Warning>
  If using Git Bash on Windows, `melos` will not be callable directly as it will have a `*.bat` extension. You can 
  update your `~/.bash_profile` with the following function to call melos with arguments:
