...
such as the `dart` and `flutter` tools and IDE plugins, than the mechanism currently being used
by default. Please read the documentation for [`usePubspecOverrides`](/configuration/overview#command/bootstrap/usePubspecOverrides)
