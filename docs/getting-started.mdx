...
Melos can also be used for projects with a single application or package to
leverage its features like versioning, publishing, and changelog generation.
This is useful when you want to use Melos tools but don't have a monorepo
structure.
...
skipped by `melos version` by default. To version an application you also need
to pass `--all` to `melos version`, or set
[`versionPrivatePackages: true`](/configuration/overview#versionprivatepackages):
...
The build number of the version of an application is retained and incremented
when it is versioned, so `1.2.3+45` becomes `1.3.0+46`, see
[Build numbers](/guides/automated-releases#build-numbers).
