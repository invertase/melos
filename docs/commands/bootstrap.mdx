...
Since the synced versions are overwritten on every bootstrap, it can be useful
to make it visible in the packages that an entry is shared. When
[`markSharedDependencies`](/configuration/overview#markshareddependencies) is
enabled, every shared entry in the `pubspec.yaml` files of the packages is
