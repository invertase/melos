...
### Build numbers when versioning

`melos version` now retains and increments an integer build number on every
version bump, so `3.0.0+11` becomes `4.0.0+12` instead of `4.0.0`.

A patch release below `1.0.0` now bumps the patch component instead of adding a
build number, so `0.1.0` becomes `0.1.1` instead of `0.1.0+1`. See
[Build numbers](/guides/automated-releases#build-numbers) for more details.
...
### `melos analyze` was removed in 7.x.x

Since `flutter/dart analyze` now runs in the full pub workspace, the
`melos analyze` command was removed in Melos 7.0.0.

The command was restored in Melos 8.1.0, where it analyzes all the packages in
a single run and supports all the package filtering options, see the
