...
Melos is able to **version** (including changelogs) and **publish** your
packages.
...
4. Filter out commits with types that don't trigger a version bump. For example,
   `chore` commits will be ignored. If at that point no commits remain, the
   package will not be versioned.
5. Determine the version upgrade required for each package based on the commit
   with the highest version bump. For example, if a package has a `feat` commit
   and a `fix` commit, the package will be versioned as a `minor` upgrade.
...
A breaking change will trigger a **major** version bump. A feature will trigger
a **minor** version bump. All other changes will trigger a **patch** version
bump.
...
versioned. This allows you to mix automatic and manual versioning.

### Dependents Versioning

By default, Melos also updates the constraints in the pubspec files of
...
### Commiting and Taging Version Changes

After updating the pubspec files and changelogs of the versioned packages, Melos
will commit the changes and add a package version tag for each package.

Package version tags have the format `<package-name>-v<version>`.

This behavior can be disabled by passing `--git-tag-version`.

#### Hook

After updating pubspec files and changelogs but before creating the version
commit the `version` scirpt in `melos.yaml` is executed. This allows you to run
custom scripts before the version commit is created.

Note that you have to stage any changes that you want to be included in the
version commit yourself.

### Changelog

Melos updates the changelog for each package that is versioned. You can disable
this behavior by passing `--no-changelog`.

See the [configuration options](/configuration/overview#commandversion) for the
`version` command for a number of options for changelog generation.

Optionally, Melos can also write a
[workspace changelog](/configuration/overview#commandversionworkspacechangelog).
...
2. Be on a machine which is authenticated with Pub (read: not possible to
   publish via CIs).
...
If your packages are private and don't publish to [pub.dev](https://pub.dev),
you can use the tag generated as git reference in your `pubspec.yaml` file and
Melos will ensure the versions are updated accordingly.
