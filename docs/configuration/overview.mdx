...
When enabled, the root directory (containing the workspace configuration) will be treated as a package and included in workspace operations such as scripts, filtering, and categorization.
...
**Note:** While we recommend keeping the root directory focused on workspace configuration, this option provides flexibility for existing project structures.
