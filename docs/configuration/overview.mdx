...
This keeps the logs of large workspaces readable, for example on a continuous
integration server. When a command such as `melos analyze`, `melos format`,
`melos test`, `melos exec` or `melos run` succeeds, nothing is printed. When it
fails, only the output of the packages or scripts that failed is printed,
followed by the summary of the failures. The results of commands like
`melos list` are always printed.

The output of a package or script is buffered until it has finished, so that it
can be dropped when it succeeded. Scripts that use `stdio: inherit` write
directly to the terminal and are therefore not affected.

If the option is specified though multiple mechanisms, the precedence from
highest to lowest is:

1. `--quiet` (or `--no-quiet`) global command line option
2. `MELOS_QUIET` environment variable
3. `quiet` in the `melos` section in the root `pubspec.yaml`
...
The maximum number of retry attempts including the first request.
...
The default is `true`. The files are never generated in a continuous integration
environment, see [IDE support](/ide-support#intellij).
...
Allows specifying the entry point and additional run arguments per Flutter app
package. This generates one IntelliJ run configuration per entry instead of the
...
The `entryPoint` field sets the Dart file to run, relative to the package root.
The default is `lib/main.dart`. The `args` field sets additional arguments to
pass to `flutter run` and can be omitted.
...
The entries of a package must have unique names, and only one of them can be
the default.

A Flutter package without a `lib/main.dart` is only treated as a Flutter app
when at least one of its entries has an `entryPoint` that exists, which is how
apps with one entry point per flavor are declared:
...
Whether the entries that are synced from the
[shared dependencies](/commands/bootstrap#shared-dependencies) are marked with
a comment in the `pubspec.yaml` files of the packages, so that it is visible
that they should be changed in the root `pubspec.yaml` file instead.

```yaml
dependencies:
  collection: ^1.18.0 # Managed by Melos, change it in the bootstrap config of the root pubspec.yaml
```

The marker is kept up to date on every bootstrap. It is removed again from
entries that are no longer shared, and from all entries when the option is
disabled. This only happens while at least one shared dependency is still
configured, since the `pubspec.yaml` files of the packages are not touched
otherwise.
...
To release from another branch, such as a hot-fix branch, change this option on
that branch. See the [Releasing from Hot-fix
...
Whether the [`sources`](/configuration/scripts#sources) of commands are ignored,
so that they run in every package without calculating or storing checksums. The
default is `false`.

The option is resolved in the following order, where the first one that is
specified wins:

1. `--ignore-sources` and `--no-ignore-sources` command line options
2. `MELOS_IGNORE_SOURCES` environment variable
3. `ignoreSources` in the workspace configuration
