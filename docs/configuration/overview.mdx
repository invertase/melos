...
Any key prefixed with `x-` is an extension field. Melos ignores these keys, so
they can be used to declare reusable [YAML anchors] without tripping up
configuration or schema validation. This follows the same convention as the
...
Extension fields are also allowed inside the `scripts` section, so an anchor can
be declared next to the scripts that alias it:
...
<Warning>
  ...
  mapping that also sets other keys.
</Warning>
