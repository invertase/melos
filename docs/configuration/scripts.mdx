...
- The scripts that a script depends on can depend on other scripts themselves.
  ...
  it depends on.
- A script that multiple scripts depend on only runs once per `melos run`
  invocation.
...
- A script can consist of only `dependsOn`, without `run`, `exec` or `steps`,
  to group other scripts under a single name.
...
on the command line, such as `--scope`, apply to all of the scripts.
