...
Scripts can be organized into logical groups, which can later be targeted or filtered using the `--group` option when running commands.
This is useful for hiding or executing only scripts that belong to certain groups.
